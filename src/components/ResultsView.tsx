                ...
                <div className="px-4 pb-4 border-t border-wine-50">
                  ...
                  <div className="mt-4 space-y-4">
                      ...
                      <div key={entry.id} className="space-y-2">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-wine-100 flex items-center justify-center text-xs font-bold text-wine-700">
                            {getParticipantName(entry.user_id)[0].toUpperCase()}
                          </div>
                          <span className="text-sm font-semibold text-wine-900">
                            {getParticipantName(entry.user_id)}
                          </span>
                          {entry.rating !== null && (
                            <span className="ml-auto flex items-center gap-1 text-sm font-bold text-wine-800">
                              <Star className="w-3.5 h-3.5 text-gold-500 fill-gold-500" />
                              {entry.rating}
                            </span>
                          )}
                          {entry.ranking !== null && (
                            <span className="text-xs bg-wine-50 text-wine-600 px-2 py-0.5 rounded-full">
                              #{entry.ranking}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="ml-9 space-y-1">
                          {guessFields.map((field) => {
                            const guessKey = `guess_${field}` as keyof TastingEntry;
                        ...
                        {(entry.appearance_notes ||
                          ...
                          entry.palate_notes ||
                          entry.overall_notes) && (
                          <div className="ml-9 text-xs text-wine-500 space-y-0.5 mt-1">
                            {entry.appearance_notes && (
                              <p>
                    ...
                    {wineEntries.length === 0 && (
                      <p className="text-xs text-wine-300 text-center py-4">
                        No entries for this wine
                      </p>
                    )}
                  </div>
                </div>
